 ...
 * extensions, and providing backwards-compatibilty for older C++ standards.
 *
 * Maybe included by C, C++, and assembly files.
 */
#ifndef SYSTEM_COMPILER_H_
...
//
// If feature checking macros are not provided by the compiler,
// we assume that the checked features are unavailable.
...
// The given function allocates memory of size argument |x| or |x| * |y| and
// returns a pointer to that memory. Argument indexes are one-based. The
// compiler can use this information to support compile-time
// __builtin_object_size checks.
#if defined(__clang__)
...
// Perform an arithmetic operation, returning "true" if the operation
// overflowed.
//
...
// code.
#if defined(__clang__) && \
    (defined(_LIBCPP_ENABLE_THREAD_SAFETY_ANNOTATIONS) || defined(_KERNEL))
...
#define __TA_ACQUIRE(...) \
    __THREAD_ANNOTATION(__acquire_capability__(__VA_ARGS__))
#define __TA_ACQUIRE_SHARED(...) \
    __THREAD_ANNOTATION(__acquire_shared_capability__(__VA_ARGS__))
#define __TA_TRY_ACQUIRE(...) \
    __THREAD_ANNOTATION(__try_acquire_capability__(__VA_ARGS__))
#define __TA_TRY_ACQUIRE_SHARED(...) \
    __THREAD_ANNOTATION(__try_acquire_shared_capability__(__VA_ARGS__))
// Neither ACQUIRED_BEFORE nor ACQUIRED_AFTER are implemented in clang. Users of
// these macros must not rely upon them to catch lock ordering bugs, and must
// treat them as documentation only. See:
// https://clang.llvm.org/docs/ThreadSafetyAnalysis.html#acquired-before-and-acquired-after-are-currently-unimplemented
#define __TA_ACQUIRED_BEFORE(...) \
    __THREAD_ANNOTATION(__acquired_before__(__VA_ARGS__))
#define __TA_ACQUIRED_AFTER(...) \
    __THREAD_ANNOTATION(__acquired_after__(__VA_ARGS__))
#define __TA_RELEASE(...) \
    __THREAD_ANNOTATION(__release_capability__(__VA_ARGS__))
#define __TA_RELEASE_SHARED(...) \
    __THREAD_ANNOTATION(__release_shared_capability__(__VA_ARGS__))
#define __TA_REQUIRES(...) \
    __THREAD_ANNOTATION(__requires_capability__(__VA_ARGS__))
#define __TA_REQUIRES_SHARED(...) \
    __THREAD_ANNOTATION(__requires_shared_capability__(__VA_ARGS__))
...
#define __TA_ASSERT_SHARED(...) \
    __THREAD_ANNOTATION(__assert_shared_capability__(__VA_ARGS__))
...
#define __TA_NO_THREAD_SAFETY_ANALYSIS \
    __THREAD_ANNOTATION(__no_thread_safety_analysis__)
